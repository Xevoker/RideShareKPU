...
const user = JSON.parse(userStr);
  ...
  // 2. Calculate ride stats
  const ridesTaken = Array.isArray(user.ridesTaken) ? user.ridesTaken : [];
  const ridesOffered = Array.isArray(user.ridesOffered) ? user.ridesOffered : [];

  // Upcoming rides = rides with date in future or today and status not cancelled
  const today = new Date();
  function isUpcoming(ride) {
    const rideDate = new Date(ride.date);
    return rideDate >= today && ride.status !== 'cancelled';
  }

  const upcomingRidesTaken = ridesTaken.filter(isUpcoming);
  ...
  // Combine upcoming ridesTaken and ridesOffered for table display (optional: show separately if you want)
  const combinedRides = [...upcomingRidesTaken, ...upcomingRidesOffered];

  if (combinedRides.length === 0) {
    const tr = document.createElement('tr');
    const td = document.createElement('td');
    td.setAttribute('colspan', 7);
    td.textContent = "No upcoming rides.";
    tr.appendChild(td);
    tableBody.appendChild(tr);
  } else {
    // Sort by date ascending
    combinedRides.sort((a, b) => new Date(a.date) - new Date(b.date));

    combinedRides.forEach(ride => {
      const tr = document.createElement('tr');
      tr.innerHTML = `
        <td>${new Date(ride.date).toLocaleDateString(undefined, { year: 'numeric', month: 'long', day: 'numeric' })}</td>
        <td>${ride.from}</td>
        <td>${ride.to}</td>
        <td>${ride.driver}</td>
        <td>${ride.seats}</td>
        <td><span class="status ${ride.status.toLowerCase()}">${capitalizeFirstLetter(ride.status)}</span></td>
        <td><a href="details.html?rideId=${ride.id}" class="btn-details">Details</a></td>
      `;
      tableBody.appendChild(tr);
    });
  }

  // 4. Populate Upcoming Events
  const upcomingEvents = Array.isArray(user.upcomingEvents) ? user.upcomingEvents : [];
  const eventsList = document.getElementById('upcomingEventsList');
  eventsList.innerHTML = '';

  if (upcomingEvents.length === 0) {
    eventsList.innerHTML = '<li>No upcoming events or promotions.</li>';
  } else {
    // Sort by date ascending
    upcomingEvents.sort((a, b) => new Date(a.date) - new Date(b.date));
    upcomingEvents.forEach(event => {
      const li = document.createElement('li');
      li.innerHTML = `<strong>${new Date(event.date).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}:</strong> ${event.title} ${event.description ? '— ' + event.description : ''}`;
